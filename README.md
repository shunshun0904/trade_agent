...
建玉があるサイクルは、A1 に到達する前に停止する
(`risk/rules.py`、`HaltReason.POSITION_OPEN`)。同時保有は最大1のため、
新規建ての検討自体が無意味だからである。
...
**損切りと利確の値はエントリー時に確定し、以後変わらない。**
トレーリングストップ、時間切れ決済、建玉中の再評価は**いずれも存在しない**。
建てた瞬間の A2 の判断が、決済まで拘束力を持つ。

**この設計上の帰結は認識しておく価値がある** —— 論拠が崩れても、価格が
どちらかの水準に触れるまでシステムは持ち続ける。A2 は提案時に
`invalidation`(この案が無効になる条件)を書いているが、**現状それを読む
コードはない**(`cycle.py` が受け取り、ガードが引用値を検証して、そこで終わり)。
建玉中の再評価を入れるかどうかは docs/OPEN-QUESTIONS.md で扱う。
