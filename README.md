...
### 建玉あり —— 決済は決定論、その上に任意の見直しが乗る

新規建ての検討は行わない。同時保有は最大1なので、建玉があるサイクルは
A1 に到達する前に停止する(`risk/rules.py`、`HaltReason.POSITION_OPEN`)。

**決済そのものは5分tickが決定論的に実行する。ここに LLM は入らない:**

```
5分tick(LLM不使用・コスト0円)
   ↓
損切り: 取引所側の stop 注文が約定 —— tick が落ちていても効く
        (取引所が保持していない場合のみ、tick が成行で売る)
   ↓
利確:   last >= take_profit で tick が PostOnly 指値を出す(メイカー)
```

仕様 §11 の「建てる能力は失ってよいが、閉じる能力は失ってはならない」は
これで構造的に満たされる。月間LLM予算を使い切っても、API が落ちても、
損切りは効く。

#### 決済レビュー(`exit_review`、既定 **OFF**)

30分の screen が、建玉があるときに**損切りと利確の水準を見直す**。
決済を実行するのは相変わらず tick で、レビューは**水準を動かすだけ**である。
...
|---|---|
| `hold` | 何も変えない |
| `raise_stop` | 損切りを**切り上げる**(現在より高い値のみ) |
| `lower_target` | 利確を**引き下げる**(現在より低い値のみ) |

**損切りを広げる・利確を遠ざける・数量を変えることはできない。**
プロンプトでの依頼ではなく `guards/deterministic.py::validate_exit` が
棄却するので、**建玉に対してリスクを増やす経路が存在しない**。

「今すぐ決済」に専用の action は無い。損切りを現在価格より上に置けば
即時決済(`protection.arm` の `close_immediately`)、利確を買い気配以下に
置けば次のtickでメイカー決済になる。既存の執行機構がそのまま使われる。

判断の中心は **`invalidation`** —— A2 がエントリー時に書いた「この案が無効に
なる条件」である。以前はこれを書かせて捨てていた(`cycle.py` が受け取り、
ガードが引用値を検証して終わり)。今は `Position` まで運ばれ、
レビューは「それが実際に起きたか」を問う。

**LLM が使えない・予算切れ・ガード棄却 —— すべて水準は1円も動かない。**

##### なぜ既定 OFF か

この機能が持ち込むリスクは「**勝ちを刈り取る**」ことである。単調制約は
1トレードの損失は守るが、期待値は守らない。そしてこれは合計値には現れず、
**レビューを経た利確が、経ていない利確より小さい**という形でしか見えない。

比較対象が無ければ永久に判定できないので、まず決定論のまま決済を数件ためて
ベースラインを取る。`get_trades` の `by_exit_reason` が
`reviewed` / `deterministic` に分けて件数と損益を出す。
