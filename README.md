...
成立したときだけ LLM を呼ぶ。1サイクル **2コール**。

```
市況分析(A1)          ← MarketSnapshot だけを見て地合いを判定
   ↓
戦略(A2)              ← 買うか見送るかを1案だけ出す。ここが唯一の判断
   ↓
合意ルール(Python)     ← buy でなければ no_trade
   ↓
サイズ算出(Python)     ← 損失上限から機械的に。モデルは触らない
   ↓
決定論ガード → check_executable → 再クオート → 冪等発注
   ↓
...
外した理由は全部同じ —— **モデルがモデルを検査する層は、決定論ガードが
既にやっていることの劣化版でしかない。**

- **検査官(A5)** 数値検証は決定論ガードが全部やっており、残るのは定性的な
  矛盾探しだけで、そこが一番当てにならない。
- **指揮官(A6)** の GO/NO-GO は、Python の関門を全部通った後に乗る
  5つ目の拒否権だった。
- **リスク査定(A4)** は Python が算出済みの数量を承認/拒否するだけで、
  しかもガードが「Python の数字と違えば棄却」していた。**変更を許されて
  いない算術に対する拒否権**である。
- **裁定(A3)** は複数案から1つを選ぶ役だった。戦略家が1体になった時点で
  選ぶものがない。
- **相互批判** は批判する相手が必要である。1案では匿名化するものがない。

A4 が担っていた「損切り位置は妥当か」という定性判断は、**A2 のプロンプトに
移した**(`agents/prompts.py` の `STRATEGY_ROLE`)。損切りを決める本人に
伝える方が、決めたあとで別のモデルに問い直すより早く、拒否権より安全である。

オーナー向けレポート本文は **Python が組み立てる**
(`orchestrator/report.py`)。材料はすべて構造化済みのフィールドなので、
モデルに書き直させる必要がない。むしろ数値が必ず入り、実際に起きたことと
食い違わない。
...
| 項目 | 月額 |
|---|---|
| LLM(Anthropic 直 API) | 1サイクル2コール。予算 **2,900円/月** |
...
回数は金額の代理変数として質が悪い(キャッシュが効くかどうかで1サイクルの
実費が大きく変わる)。荒れた日は多く、静かな日は少なく
