...
```bash
cd ~ && curl -fsSL https://raw.githubusercontent.com/shunshun0904/trade_agent/claude/trade-agent-spec-mtwldk/scripts/deploy.sh | bash -s -- --yes
```

`--yes`(または `TA_YES=1`)は、既存のデプロイに対して**安全な答えを自動で選ぶ**。

| 質問 | `--yes` の答え | 理由 |
|---|---|---|
| SSM のシークレットを置き換えるか | **いいえ** | 再デプロイで消したいものではない |
| MCP トークンを入れ替えるか | **いいえ** | claude.ai 側の登録が無効になる |
| ビルド成果物を消すか | **はい** | 次の `sam build` が作り直す |
| **失敗スタックを削除するか** | **拒否して終了** | 破壊的な判断は無人で行わない |
| 孤児ロググループを消すか | **いいえ** | 同上 |

メールアドレスは**既存スタックのパラメータから読む**ので、環境変数も入力も不要。
初回デプロイや別環境では `TA_OWNER_EMAIL` で渡せる。

**ロールバックしたスタックの掃除だけは `--yes` を外して実行すること。**
空の抜け殻とはいえ削除であり、無人で走らせる操作ではない。
